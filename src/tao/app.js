...
TAO.addInterceptHandler({ o: 'portal' }, (tao, { portal }) =>
  agent.setToken(portal.token)
);
...
const triggerAppLoad = (tao, { portal }) =>
  new AppCtx('app', 'load', 'portal', { portal });
TAO.addInlineHandler({ t: 'user', a: 'enter', o: 'portal' }, triggerAppLoad);

TAO.addInlineHandler(
  { t: 'user', a: 'missing', o: 'portal' },
  () => new AppCtx('app', 'load', 'anon')
);
...
TAO.addInlineHandler({ t: 'app', a: 'load' }, (tao, data) => {
  ...
  return locationToAC(
    history.location,
    tao.o === 'portal' ? data.portal.token : undefined
  );
});

TAO.addAsyncHandler(
  { t: 'home', a: 'enter', o: 'anon' },
  (tao, data) => new AppCtx('article', 'find', 'anon')
);

TAO.addAsyncHandler(
  { t: 'home', a: 'enter', o: 'portal' },
  (tao, { portal }) =>
    new AppCtx('article', 'find', 'portal', { find: { feed: true }, portal })
);

TAO.addAsyncHandler(
  { t: 'home', a: 'enter' },
  (tao, data) =>
    new AppCtx('tag', 'find', tao.o, {
      action: { popular: true },
      orient: data[tao.o],
    })
);
