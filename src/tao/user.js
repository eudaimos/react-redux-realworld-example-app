...
TAO.addInterceptHandler(
  ...
  (tao, { portal }) => {
    if (!portal || !portal.token) {
      return new AppCtx('user', 'fail', 'anon', {
        fail: { auth: true, errors: { token: ['missing token'] } },
      });
    }
  }
);
...
TAO.addInlineHandler({ t: 'user', a: 'find', o: 'portal' }, (tao, data) => {
  return agent.Auth.current()
    .then(({ user }) => {
      return new AppCtx('user', 'enter', 'portal', user, null, {
        token: user.token,
      });
    }) //;
    .catch((err) => {
      const { errors } =
        err.response && err.response.body ? err.response.body : {};
      return new AppCtx('user', 'missing', 'portal', {
        missing: { auth: true, errors },
        portal: data.portal,
      });
    });
});

TAO.addInterceptHandler({ t: 'user', a: 'missing', o: 'portal' }, () => {
...
TAO.addInlineHandler({ t: 'user', a: 'update', o: 'portal' }, (tao, data) => {
  ...
  return agent.Auth.save(update)
    .then(
      ({ user }) =>
        new AppCtx('user', 'stored', 'portal', user, null, {
          token: user.token,
        })
    )
    .catch((err) => {
